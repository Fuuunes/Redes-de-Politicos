class RelationsController < ApplicationController
  ...
  def suggest
    ...
    journal_number   = params[:journal_number].to_i  if(params[:journal_number] != "" )
    ...
    if !error.nil?
      ...
      case  journal_number
        when 0
          json = ElNorte.suggestions_for(cand_nicknames,key_words,ally_nicknames) 
        when 1
          json = Milenio.suggestions_for(cand_nicknames,key_words,ally_nicknames) 
        when 2
          json = ElPorvenir.suggestions_for(cand_nicknames,key_words,ally_nicknames) 
        when 3  
          json = ABC.suggestions_for(cand_nicknames,key_words,ally_nicknames)
        when 4
          json = ReporteIndigo.suggestions_for(cand_nicknames,key_words,ally_nicknames)
        when 5
          json = Reforma.suggestions_for(cand_nicknames,key_words,ally_nicknames)
        when 6
          json = ElUniversal.suggestions_for(cand_nicknames,key_words,ally_nicknames)
        when 7
          json = Excelsior.suggestions_for(cand_nicknames,key_words,ally_nicknames)
        when 8
          json = LaJornada.suggestions_for(cand_nicknames,key_words,ally_nicknames)
        when 9
          json = Notimex.suggestions_for(cand_nicknames,key_words,ally_nicknames)
        when 10
          json = AgenciaEfe.suggestions_for(cand_nicknames,key_words,ally_nicknames)
        else
          json = ElUniversal.suggestions_for(cand_nicknames,key_words,ally_nicknames)
          #binding.pry
      end
